...
@Component({
  selector: 'app-filtres-lignes-pc',
  templateUrl: './filtres-lignes-pc.component.html',
  styleUrls: ['./filtres-lignes-pc.component.scss']
})
export class FiltresLignesPcComponent {
  ...
  ngOnInit() {
    // ... votre code existant
    this.statusEntries = Object.entries(this.statusColorMap).map(([status, color]) => ({ status, color }));
  }
  ...
  onCategrorieValueChange() {
    ...
    console.log("statuses: ", this.selectedStatutsFilters);
    console.log("installation: ", this.selectedInstallation);
    console.log("ouvrage: ", this.selectedOuvrage);
    console.log("entite: ", this.selectedEntites);
    console.log("responsableInitiales: ", this.selectedResponsable);
