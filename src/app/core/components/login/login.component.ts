  ...
  async onLoginNew(initiales: string) {
    if (initiales) {
      ...
      if (this.credentialsAreValid == true) {
        const redirectUrl = this.authService.redirectUrl ? this.authService.redirectUrl : '/projets';
        this.authService.redirectUrl = null; // Réinitialiser l'URL de redirection
